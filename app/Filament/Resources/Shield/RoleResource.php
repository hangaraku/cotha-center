...
class RoleResource extends Resource implements HasShieldPermissions
{
            ...
            $entities[] = Forms\Components\Section::make(FilamentShield::getLocalizedResourceLabel($entity['fqcn']))
                ...
                ->schema([
                    Forms\Components\CheckboxList::make($entity['resource'])
                        ...
                        ->descriptions(fn (): array => static::getResourcePermissionDescriptions($entity))
                        ->live()
    ...
    public static function getResourcePermissionOptions(array $entity): array
    {
        return collect(Utils::getResourcePermissionPrefixes($entity['fqcn']))
            ->flatMap(function ($permission) use ($entity) {
                $permissionKey = $permission . '_' . $entity['resource'];
                $permissionLabel = FilamentShield::getLocalizedResourcePermissionLabel($permission);
                return [
    ...
    public static function getResourcePermissionDescriptions(array $entity): array
    {
        return collect(Utils::getResourcePermissionPrefixes($entity['fqcn']))
            ->flatMap(function ($permission) use ($entity) {
                $permissionKey = $permission . '_' . $entity['resource'];
                $tooltip = static::getPermissionTooltip($permission);
                return [
                    $permissionKey => $tooltip,
                ];
            })
            ->toArray();
    }
    
    protected static function getPermissionTooltip(string $permission): string
    {
        $tooltips = [
            'view' => '👁️ View individual records (open single record details)',
            'view_any' => '📋 View list of all records (see the main table/list page)',
            'create' => '➕ Create new records (add new items)',
            'update' => '✏️ Edit existing records (modify current items)',
            'restore' => '🔄 Restore individual soft-deleted records (recover from trash)',
            'restore_any' => '🔄 Restore multiple soft-deleted records (bulk recover from trash)',
            'replicate' => '📋 Duplicate/copy records (create copies of existing items)',
            'reorder' => '📝 Change the order of records (drag and drop to reorder)',
            'delete' => '🗑️ Delete individual records (move to trash)',
            'delete_any' => '🗑️ Delete multiple records at once (bulk delete to trash)',
            'force_delete' => '💀 Permanently delete individual records (bypass trash, cannot recover)',
            'force_delete_any' => '💀 Permanently delete multiple records (bulk permanent delete, cannot recover)',
        ];
        
        return $tooltips[$permission] ?? '';
    }
